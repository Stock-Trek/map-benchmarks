...
fn bench_out_of_the_box<Map, K>(
    ...
    Map: BenchMapNew<K, u64> + BenchMapMutInsert<K, u64> + BenchMapMutGetOrInsert<K, u64>,
    K: Clone + Hash + Eq,
{
...
fn get_or_insert(c: &mut Criterion) {
    ...
    {
        let mut group = bench_group!(c, format!("get-or-insert/{OUT_OF_THE_BOX}/u64"));
    ...
    {
        let mut group = bench_group!(c, format!("get-or-insert/{SAME_HASHER}/u64"));
        group.throughput(Throughput::Elements(op_count as u64));
    ...
    {
        let mut group = bench_group!(c, format!("get-or-insert/{OUT_OF_THE_BOX}/String<32>"));
        group.throughput(Throughput::Elements(op_count as u64));
    ...
    {
        let mut group = bench_group!(c, format!("get-or-insert/{SAME_HASHER}/String<32>"));
        group.throughput(Throughput::Elements(op_count as u64));
