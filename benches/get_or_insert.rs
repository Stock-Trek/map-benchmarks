...
fn get_or_insert(c: &mut Criterion) {
    ...
    // Each map uses its default hasher
    {
        ...
        bench_out_of_the_box::<RustCHashBenchMap<u64, u64>>("rustc-hash", &mut group, &map_data);
    ...
    // Every map that supports a custom hasher uses the same CommonHasher
    {
        ...
        // bench_same_hasher::<CrossbeamSkiplistBenchMap<u64, u64, CommonHasher>>("crossbeam-skiplist", &mut group, &map_data); // doesn't allow setting hasher
