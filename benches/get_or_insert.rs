...
fn get_or_insert(c: &mut Criterion) {
    ...
    for entry_count in DEFAULT_ENTRY_COUNTS {
        // Cap the hit-path (existing) and insert-path (missing) key counts at
        // the map size: MapGen::generate asserts existing_key_count <=
        // entry_count, and a 1K map cannot hold 10K distinct existing keys.
        // At sizes >= DEFAULT_OP_COUNT this is 10K hits + 10K inserts.
        let op_key_count = (*entry_count).min(DEFAULT_OP_COUNT);
        let existing_key_count = op_key_count;
        let missing_key_count = op_key_count;
