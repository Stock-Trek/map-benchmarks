    ...
    for (missing_key_count, entry_count_name) in GROWTH_ENTRY_COUNTS {
        ...
        // out of the box, u64 keys
        {
            let mut group = bench_group!(
                c,
                format!("growth/map-size-{entry_count_name}/{OUT_OF_THE_BOX}/u64")
            );
        ...
        // CommonHasher, u64 keys
        {
            let mut group = bench_group!(
                c,
                format!("growth/map-size-{entry_count_name}/{SAME_HASHER}/u64")
            );
            group.throughput(Throughput::Elements(*missing_key_count as u64));
        ...
        // out of the box, String<32> keys
        {
            let mut group = bench_group!(
                c,
                format!("growth/map-size-{entry_count_name}/{OUT_OF_THE_BOX}/String<32>")
            );
            group.throughput(Throughput::Elements(*missing_key_count as u64));
        ...
        // CommonHasher, String<32> keys
        {
            let mut group = bench_group!(
                c,
                format!("growth/map-size-{entry_count_name}/{SAME_HASHER}/String<32>")
            );
            group.throughput(Throughput::Elements(*missing_key_count as u64));
