...
fn lookup_miss(c: &mut Criterion) {
    ...
    {
        let mut group = bench_group!(c, format!("lookup-miss/{OUT_OF_THE_BOX}/u64"));
    ...
    {
        let mut group = bench_group!(c, format!("lookup-miss/{SAME_HASHER}/u64"));
        group.throughput(Throughput::Elements(missing_key_count as u64));
    ...
    {
        let mut group = bench_group!(c, format!("lookup-miss/{OUT_OF_THE_BOX}/String<32>"));
        group.throughput(Throughput::Elements(missing_key_count as u64));
    ...
    {
        let mut group = bench_group!(c, format!("lookup-miss/{SAME_HASHER}/String<32>"));
        group.throughput(Throughput::Elements(missing_key_count as u64));
